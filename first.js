...
const output=countingWords(inputtext);
console.log(output);


function countingWords(text){
    const clearText=text.toLowerCase().replace(/[.,]/g, '').split(/\s+/);
    let wordCount={};
    let Count=null;
    for(word of clearText){
        if (!(/^\d/.test(word))) {
            const firstLetter = word.charAt(0);
            Count+=1;
            if (/[a-z]/.test(firstLetter)) {
                wordCount[firstLetter] = (wordCount[firstLetter] || 0) + 1;
            }
    ...
    if (!(/^\d/.test(word))) {
        for(let i=0;i<clearText.length;i++){
            if(eachword[clearText[i]]==undefined){
                eachword[clearText[i]]=1;
            }else{
                eachword[clearText[i]]++; 
            }
        }
    }
